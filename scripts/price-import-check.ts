...
import { clientSellPrice, discountBreakEvenPercent, priceBreakdown } from "../src/lib/pricing";
...
async function main() {
  ...
  const zeroMarkup = markupForPrice(1500, [{ id: "z", min: 0, max: null, markupPercent: 0 }], 18);
  assert(zeroMarkup === 0, `наценка 0 не должна стать 18, got ${zeroMarkup}`);
  const classic = priceBreakdown(1000, 16, 8);
  assert(classic.markupAmount === 160, `markup ${classic.markupAmount}`);
  assert(classic.discountAmount === 92.8, `discount ${classic.discountAmount}`);
  assert(classic.sell === 1067.2, `classic sell ${classic.sell}`);
  assert(!classic.belowBuy, "1000+16%-8% не ниже закупа");
  const below = priceBreakdown(1000, 10, 20);
  assert(below.belowBuy, "10% наценка и 20% скидка ниже закупа");
